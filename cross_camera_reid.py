...
ATTR_ENABLED = True
ATTR_MODEL_NAME = "ViT-B-32"
ATTR_MODEL_PRETRAINED = "openai"
...
class CLIPAttributeExtractor:
    """CLIP으로 person crop에서 binary attribute 확률을 추출한다."""

    def __init__(
        self,
        device: str = "cpu",
        model_name: str = ATTR_MODEL_NAME,
        pretrained: str = ATTR_MODEL_PRETRAINED,
        ...
        try:
            ...
            raise ImportError(
                "open_clip_torch 또는 Pillow가 설치되어 있지 않습니다.\n"
                "아래 명령을 먼저 실행해 주세요:\n"
        ...
        prompt_map = {
            "backpack": ("person wearing a backpack", "person without a backpack"),
            "hat": ("person wearing a hat", "person without a hat"),
            "upper_black": ("person wearing black upper clothes", "person wearing non-black upper clothes"),
            "upper_white": ("person wearing white upper clothes", "person wearing non-white upper clothes"),
            "upper_gray": ("person wearing gray upper clothes", "person wearing non-gray upper clothes"),
            "upper_red": ("person wearing red upper clothes", "person wearing non-red upper clothes"),
            "upper_orange": ("person wearing orange upper clothes", "person wearing non-orange upper clothes"),
            "upper_yellow": ("person wearing yellow upper clothes", "person wearing non-yellow upper clothes"),
            ...
            "lower_gray": ("person wearing gray lower clothes", "person wearing non-gray lower clothes"),
            "lower_red": ("person wearing red lower clothes", "person wearing non-red lower clothes"),
            "lower_orange": ("person wearing orange lower clothes", "person wearing non-orange lower clothes"),
            "lower_yellow": ("person wearing yellow lower clothes", "person wearing non-yellow lower clothes"),
            ...
            "backpack_gray": ("person wearing a gray backpack", "person wearing a non-gray backpack"),
            "backpack_red": ("person wearing a red backpack", "person wearing a non-red backpack"),
            "backpack_orange": ("person wearing an orange backpack", "person wearing a non-orange backpack"),
            "backpack_yellow": ("person wearing a yellow backpack", "person wearing a non-yellow backpack"),
        ...
        with torch.no_grad():
            for attr in self.attributes:
                pos, neg = prompt_map.get(attr, (f"person with {attr}", f"person without {attr}"))
                tokens = self.tokenizer([pos, neg]).to(self.device)
                text_feat = self.model.encode_text(tokens)
                text_feat = text_feat / text_feat.norm(dim=-1, keepdim=True)
                self.text_features[attr] = text_feat
    ...
    def extract(self, bgr_crop: np.ndarray) -> Optional[np.ndarray]:
        ...
        probs = []
        for attr in self.attributes:
            text_feat = self.text_features.get(attr)
...
class AttributeStore:
    """track별 attribute vector buffer를 유지하고 평균 attribute를 반환."""
    ...
    def update(self, key: Union[int, str], attrs: np.ndarray, frame_idx: int):
        self.buffers[key].append(attrs.astype(np.float32))
        self.last_seen[key] = frame_idx
    ...
    def get_mean(self, key: Union[int, str]) -> Optional[np.ndarray]:
        ...
        if not buf:
            return None
        mean_attr = np.mean(np.stack(list(buf), axis=0), axis=0)
...
def attribute_similarity(a: Optional[np.ndarray], b: Optional[np.ndarray]) -> Optional[float]:
    if a is None or b is None:
        return None
    if a.shape != b.shape:
        return None
...
def attribute_confident_mismatch(
    ...
    conf: float = ATTR_HARD_CONF,
    margin: float = ATTR_HARD_MARGIN,
) -> bool:
    if a is None or b is None:
        return False
    if a.shape != b.shape:
...
class Gallery:
    ...
    def __init__(
        ...
        self.attr_weight = float(max(0.0, min(1.0, attr_weight)))
        self.attr_enabled = bool(attr_enabled)
        self.attr_hard_filter = bool(attr_hard_filter)
    ...
    def _distance_feature_sets(
        ...
        if set_sim is None:
            set_sim = mean_sim
        osnet_sim = self.mean_weight * mean_sim + self.set_weight * set_sim

        if self.attr_enabled:
            if self.attr_hard_filter and attribute_confident_mismatch(attr_a, attr_b, self.attr_hard_conf, self.attr_hard_margin):
                return None
            attr_sim = attribute_similarity(attr_a, attr_b)
        else:
            attr_sim = None
...
def run(
    ...
    if attr_enabled:
        try:
            print("[INFO] Loading CLIP attribute extractor ...")
            attr_extractor = CLIPAttributeExtractor(device=device, model_name=attr_model_name, pretrained=attr_model_pretrained)
        except Exception as e:
            print("[WARN] Attribute extractor disabled due to error.")
            print(f"[WARN] Reason: {e}")
            attr_extractor = None
            attr_enabled = False
...
if __name__ == "__main__":
    ...
    parser.add_argument("--no_attr", action="store_true", help="CLIP attribute extractor를 비활성화합니다.")
    parser.add_argument("--attr_weight", type=float, default=ATTR_WEIGHT, help=f"attribute soft-score 가중치 (기본: {ATTR_WEIGHT})")
    parser.add_argument("--attr_hard_conf", type=float, default=ATTR_HARD_CONF, help=f"attribute hard-filter 신뢰도 기준 (기본: {ATTR_HARD_CONF})")
    ...
    if use_dir:
        ...
        for idx, (path_a, path_b) in enumerate(pairs, 1):
            ...
            run(
                ...
                attr_enabled=not args.no_attr,
                attr_weight=args.attr_weight,
                attr_hard_filter=True,
                attr_hard_conf=args.attr_hard_conf,
